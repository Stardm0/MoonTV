...
export const changelog: ChangelogEntry[] = [
  {
    ...
    date: "2026-09-24",
    added: [
    "管理台「私人影库」支持一键「清空配置」: 直接下发清空指令并即时刷新表单与状态徽章, 修复\"删掉再保存看起来没效果\"",
    "新增站点级开关「允许个人影库覆盖站点级配置」: 关掉后全站强制使用后台那一份, 个人在影库页填的地址不再生效(默认保持原行为)",
    "管理台检测本浏览器的个人影库配置: 命中时黄条提示\"会覆盖站点级配置\", 并提供「清除本浏览器的个人配置」一键清除",
    "影库页来源徽章补全: 个人配置生效时显示「个人配置（仅本浏览器）」, 与「管理员已配置」并列可见"
    ],
    changed: [
  ...
  },
  {
    ...
    fixed: [
    "影库多级目录「按剧集播放」报「未找到匹配结果」: 目录详情改为递归收集子目录里的视频(深度3层/列目录50次/视频500个上限), 季与子文件夹自动摊平成选集, 标题带相对路径",
    "网盘列表行移除「按剧集播放」: 网盘是容器不是剧集, 只保留「进入」(网盘根直接是视频文件的场景仍可经普通列表播放)"
    ]
  },
  {
    version: "4.3.10",
    date: "2026-09-24",
    added: [
    "影库多网盘列表: OpenList 挂载多个网盘时, 根目录把顶层目录单独列成「挂载的网盘」(云图标 + 进入/按剧集播放), 文件仍在下方列表",
    "网盘快速切换: 进入子目录后面包屑右侧出现「切换网盘」下拉, 任意层级一键跳到其它网盘或根目录, 不用逐级返回"
    ],
    changed: [
    "根路径支持子目录(如 /media): 「根目录」「上级」按钮不再越过配置的根路径"
    ],
    fixed: [
      // 无修复内容
    ]
  ...
  },
  {
    ...
    date: "2026-09-24",
    added: [
    "影库封面图片代理: 封面统一走 `/api/library-image` 由服务端补令牌与签名, 浏览器不接触影库凭据",
    "影库浏览页列表显示缩略图: 图片文件显示自身, 视频文件按命名约定套用同级封面(poster/同名图等), 不额外请求影库",
    "播放页详情补齐影库封面: 目录复用已列出的条目找封面, 单集影片自动到父目录找同名/通用封面"
    ],
    changed: [
      // 无变更内容
    ],
    fixed: [
    "从影库页直接播放(或经历史/追更进入)时不再依赖搜索恰好命中: 播放页对影库条目直接拉取详情, 修复「未找到匹配结果」"
    ]
  },
  {
    version: "4.3.6",
    date: "2026-09-23",
    added: [
    "管理台新增「私人影库」配置: 管理员填好地址与令牌后全站用户自动可用, 令牌只存服务端不下发浏览器",
    "个人在「影库」页填写的配置仍然优先, 没填的用户自动使用站点级配置",
    "搜索混排私人影库: 已配置影库时, 搜索结果自动并入影库条目(来源标记「私人影库」), 流式/非流式均支持",
    "影库条目没有海报时按标题生成稳定的渐变色块封面(首字 + 同色系), 不再显示空白卡片"
    ],
    changed: [
    "影库页在管理员已配置时显示来源标记, 可一键改用/断开个人配置",
  ...
  },
  {
    ...
    changed: [
    "侧边栏全局常驻: 跳转豆瓣/搜索/榜单页不再回退顶部导航, 播放页与管理台除外",
    "搜索回到侧边栏一级菜单, 单独打开搜索页并按路由高亮当前项",
    "搜索页桌面端恢复搜索框与源选择器, 新增返回按钮",
    "用户菜单弹出位置跟随按钮, 不再固定右上角"
    ],
    fixed: [
    "首页搜索框左侧补回搜索源选择器"
    ]
